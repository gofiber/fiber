...
func Test_Ctx_Immutable_CommonMethods(t *testing.T) {
	t.Parallel()
	...
	app.Post("/users/:id", func(c *Ctx) error {
		...
		snapshots[idx] = immutableSnapshot{
			Param:        c.Params("id"),
			Query:        c.Query("q"),
			Header:       c.Get("X-Test"),
			Cookie:       c.Cookies("ck"),
			...
			Queries:      c.Queries(),
			HeaderValues: reqHeaders["X-Multi"],
	...
	req1 := httptest.NewRequest(MethodPost, "http://alpha.example.com/users/first?q=one&multi=a&multi=b", strings.NewReader("form=alpha"))
	req1.Header.Set(HeaderContentType, MIMEApplicationForm)
	req1.Header.Set("X-Test", "header-one")
	req1.Header.Add("X-Multi", "one-a")
	req1.Header.Add("X-Multi", "one-b")
	req1.Header.Set(HeaderXForwardedFor, "203.0.113.1, 203.0.113.2")
	req1.AddCookie(&http.Cookie{Name: "ck", Value: "cookie-one"})

	resp, err := app.Test(req1)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, StatusOK, resp.StatusCode)

	req2 := httptest.NewRequest(MethodPost, "http://beta.example.net/users/second?q=two&multi=c&multi=d", strings.NewReader("form=beta"))
	req2.Header.Set(HeaderContentType, MIMEApplicationForm)
	req2.Header.Set("X-Test", "header-two")
	req2.Header.Add("X-Multi", "two-a")
	req2.Header.Add("X-Multi", "two-b")
	req2.Header.Set(HeaderXForwardedFor, "198.51.100.10, 198.51.100.11")
	req2.AddCookie(&http.Cookie{Name: "ck", Value: "cookie-two"})

	resp, err = app.Test(req2)
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, StatusOK, resp.StatusCode)

	expectedSnapshots := [2]immutableSnapshot{
		{
			Param:        "first",
			Query:        "one",
			Header:       "header-one",
			Cookie:       "cookie-one",
			OriginalURL:  "http://alpha.example.com/users/first?q=one&multi=a&multi=b",
			Path:         "/users/first",
			BaseURL:      "http://alpha.example.com",
			Hostname:     "alpha.example.com",
			FormValue:    "alpha",
			Queries:      map[string]string{"multi": "b", "q": "one"},
			HeaderValues: []string{"one-a", "one-b"},
			ParamsMap:    map[string]string{"id": "first"},
			Subdomains:   []string{"alpha"},
		...
		},
		{
			...
			Header:       "header-two",
			Cookie:       "cookie-two",
			OriginalURL:  "http://beta.example.net/users/second?q=two&multi=c&multi=d",
			Path:         "/users/second",
			BaseURL:      "http://beta.example.net",
			Hostname:     "beta.example.net",
			FormValue:    "beta",
			Queries:      map[string]string{"multi": "d", "q": "two"},
			HeaderValues: []string{"two-a", "two-b"},
			ParamsMap:    map[string]string{"id": "second"},
			Subdomains:   []string{"beta"},
			IPs:          []string{"198.51.100.10", "198.51.100.11"},
		},
	}

	utils.AssertEqual(t, expectedSnapshots, snapshots)
	utils.AssertEqual(t, "form=alpha", string(bodies[0]))
	utils.AssertEqual(t, "form=beta", string(bodies[1]))
}

func Test_Ctx_Immutable_IsolatedPointers(t *testing.T) {
	t.Parallel()
	...
	app.Post("/users/:id", func(c *Ctx) error {
		...
		header := c.Get("X-Test")
		headerPeek := c.fasthttp.Request.Header.Peek("X-Test")
		...
		query := c.Query("q")
		queryPeek := c.fasthttp.QueryArgs().Peek("q")
		...
		cookie := c.Cookies("ck")
		cookiePeek := c.fasthttp.Request.Header.Cookie("ck")
		...
		headerValues := c.GetReqHeaders()["X-Multi"]
		headerPeekFirst := c.fasthttp.Request.Header.PeekBytes([]byte("X-Multi"))
		...
		if paramPtr == pathStringPtr {
			t.Fatalf("expected param pointer to differ from path string buffer")
		}

		originalURL := c.OriginalURL()
		originalRaw := c.fasthttp.Request.Header.RequestURI()
		...
		hostname := c.Hostname()
		hostRaw := c.fasthttp.Request.URI().Host()
		...
		ips := c.IPs()
		xff := c.fasthttp.Request.Header.Peek(HeaderXForwardedFor)
		...
		formRaw := c.fasthttp.FormValue("form")
		...
		respHeader := c.GetRespHeader("X-Reply")
		respPeek := c.fasthttp.Response.Header.Peek("X-Reply")
		...
		respHeaders := c.GetRespHeaders()["X-Reply"]
	...
	req := httptest.NewRequest(MethodPost, "http://alpha.example.com/users/first?q=test", strings.NewReader("form=value"))
	req.Header.Set("X-Test", "value")
	req.Header.Add("X-Multi", "multi-value")
	req.Header.Add("X-Multi", "multi-second")
	req.Header.Set(HeaderContentType, MIMEApplicationForm)
	req.Header.Set(HeaderXForwardedFor, "203.0.113.1, 203.0.113.2")
	req.AddCookie(&http.Cookie{Name: "ck", Value: "cookie"})
...
func Test_Ctx_Immutable_MultipartValuesAreCopied(t *testing.T) {
	t.Parallel()
	...
	buf := &bytes.Buffer{}
	writer := multipart.NewWriter(buf)
	if err := writer.WriteField("field", "alpha"); err != nil {
	...
	req := httptest.NewRequest(MethodPost, "http://example.com/upload", buf)
	req.Header.Set(HeaderContentType, writer.FormDataContentType())
