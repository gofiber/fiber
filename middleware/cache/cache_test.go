...
func Test_Cache_WithNoCacheRequestDirective(t *testing.T) {
	...
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheHit, noCacheResp.Header.Get("X-Cache"))
	utils.AssertEqual(t, []byte("1"), noCacheBody)
	// Response not cached, returns cached response, entry = 2
	...
	utils.AssertEqual(t, nil, err)
	utils.AssertEqual(t, cacheHit, noCacheResp1.Header.Get("X-Cache"))
	utils.AssertEqual(t, []byte("1"), noCacheBody1)
	// Response not cached, returns cached response, entry = 2
	...
	utils.AssertEqual(t, []byte("1"), cachedBody1)
