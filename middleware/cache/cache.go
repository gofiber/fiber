...
func New(config ...Config) fiber.Handler {
	...
	return func(c fiber.Ctx) error {
		...
		e = manager.acquire()
		// Cache response
		e.body = c.CopyBytes(c.Response().Body())
		e.status = c.Response().StatusCode()
		e.ctype = c.CopyBytes(c.Response().Header.ContentType())
		e.cencoding = c.CopyBytes(c.Response().Header.Peek(fiber.HeaderContentEncoding))
		...
		if cfg.StoreResponseHeaders {
			...
			for key, value := range c.Response().Header.All() {
				...
				if _, ok := ignoreHeaders[keyS]; !ok {
					e.headers[keyS] = c.CopyBytes(value)
				}
			}
		}
