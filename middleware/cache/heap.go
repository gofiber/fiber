...
func (h *indexedHeap) removeInternal(realIdx int) (key string, size uint) {
...
func (h *indexedHeap) remove(idx int) (key string, size uint) {
	return h.removeInternal(h.indices[idx])
}

// Remove entry with lowest expiration time
func (h *indexedHeap) removeFirst() (key string, size uint) {
