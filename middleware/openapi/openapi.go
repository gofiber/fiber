...
func New(config ...Config) fiber.Handler {
	...
	return func(c fiber.Ctx) error {
		...
		case uiTarget != "" && equal(request, uiTarget):
			...
			cacheMu.Lock()
			cache := cacheFor(app)
			data, ok := cache.uiPages[targetPath]
			if !ok {
				var err error
...
func resolveTargets(c fiber.Ctx, specPath, uiPath string, equal func(a, b string) bool) (string, string) {
	...
	// A Use mount whose prefix already ends in a configured path IS the target
	// (e.g. app.Use("/v1/openapi.json", openapi.New())); strip the suffix so it
	// is not doubled.
	switch {
	case hasSuffix(prefix, specPath, equal):
		prefix = prefix[:len(prefix)-len(specPath)]
	case uiPath != "" && hasSuffix(prefix, uiPath, equal):
		prefix = prefix[:len(prefix)-len(uiPath)]
	}
...
func generateSpec(routes []fiber.Route, cfg *Config) openAPISpec {
	...
	// seenHierarchies tracks emitted path templates with parameter names
	// blanked ("/files/{}"), per method: the OpenAPI spec forbids templated
	// paths that are identical up to parameter names.
	seenHierarchies := make(map[string]struct{})
	...
	for i := range routes {
		...
		for _, variant := range variants {
			...
			hierarchyKey := normalizePathHierarchy(variant.Path) + "|" + methodLower
			if _, exists := seenHierarchies[hierarchyKey]; exists {
				continue
			}
			seenHierarchies[hierarchyKey] = struct{}{}
