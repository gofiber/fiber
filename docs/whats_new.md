...
- Added `TLSConfig` to `ListenConfig` so external providers can supply certificates via `GetCertificate`. `TLSConfig` is cloned and cannot be combined with `CertFile`/`CertKeyFile` or `AutoCertManager`.
