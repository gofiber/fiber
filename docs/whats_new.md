...
- **SafeBytes / SafeString**: Helpers that copy values only when `Immutable` is enabled and data still references request or response buffers. Access via `c.App().SafeString` and `c.App().SafeBytes`.
