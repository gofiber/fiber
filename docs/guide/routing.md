...
To keep matching deterministic, Fiber assigns every leading non-greedy parameter in that chain a fixed length of **one character**, and the **last** parameter is greedy (it takes the remainder of the segment up to the next delimiter or end of path). This is why `/:sign:param` against `/@v1` yields `sign=@` and `param=v1`, not `sign=@v` and `param=1`.
