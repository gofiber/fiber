...
- **Set `ModelMap` on every cross-dialect upstream.** A translated request keeps the client's model name; without a mapping the foreign provider answers 404 "unknown model", which — like every non-retryable response — is relayed verbatim without failing over. The mixed-chain example above is the template.
- The `AzureOpenAI` preset does **not** declare a dialect: Azure's deployment-scoped chat paths (`/openai/deployments/{name}/chat/completions?api-version=...`) cannot be synthesized by translation, so a translated request would 404. Opt in manually only when fronting an Azure endpoint that serves plain `/v1/chat/completions`.
- A chat request whose body cannot be decoded to JSON (an unsupported Content-Encoding such as `br`/`zstd`, or no body at all) cannot be translated; if no same-dialect upstream can take it verbatim, the gateway answers 400 without contacting an upstream.
- A truncated upstream stream (EOF without the dialect's terminator) is reported to the client as an explicit error event/object — never fabricated into a "completed" message — and recorded as an error in `UsageEvent.Err`.
