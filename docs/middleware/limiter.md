...
Window accounting is whole-second, so a returned duration below one second is treated as a one-second window.
...
| ExpirationFunc         | `func(fiber.Ctx) time.Duration` | Function that calculates the expiration duration dynamically. Values below one second are floored to one second. | A function that returns `cfg.Expiration` |
