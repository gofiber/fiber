...
for. `Queries` are merged into whatever query the composed path already holds
and placed ahead of any fragment, so a `Params` value carrying `?` or `#`
cannot absorb or discard them.
