...
It is important to use the struct tag `uri` or `params`. For example, if you want to parse a path parameter with a field called `Pass`, you would use a struct field with `uri:"pass"` or `params:"pass"`.
