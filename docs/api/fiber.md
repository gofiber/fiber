...
| <Reference id="skipunmatchedroutes">SkipUnmatchedRoutes</Reference>                   | `bool`                                                          | When enabled, requests to paths without a registered endpoint return 404 immediately, skipping the middleware chain entirely. This is useful for reducing unnecessary processing of requests from bots, scanners, or mistyped URLs. Costs one extra route-tree lookup per request — enable only if unmatched traffic is significant.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `false`                                                                |
