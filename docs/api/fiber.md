...
| <Reference id="passlocalstocontext">PassLocalsToContext</Reference>                   | `bool`                                                          | Controls whether context helpers also propagate values into and read values from the request `context.Context` for Fiber-backed contexts. `StoreInContext` always writes to `c.Locals()`, and when this option is enabled it also writes to `c.Context()`. `ValueFromContext` prefers `c.Context()` when enabled and otherwise reads from `c.Locals()`. | `false`                                                                |
