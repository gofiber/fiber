...
| <Reference id="disableflashmessages">DisableFlashMessages</Reference>                 | `bool`                                                          | Disables redirect flash messages: `Redirect().With` and `WithInput` set no cookie, `Messages` and `OldInput` report nothing, and the scan of every request's headers for an incoming `fiber_flash` cookie is skipped. Flash messages travel in that cookie, so only clients that keep cookies across a redirect (browsers, typically) ever receive them; deployments whose clients do not can turn them off. | `false`                                                                |
...
| <Reference id="skipunmatchedroutes">SkipUnmatchedRoutes</Reference>                   | `bool`                                                          | When enabled, requests whose path and method match no registered route are answered with `404` (or `405` when the path exists for other methods) before the middleware chain runs, avoiding work on requests to unregistered paths (bots, scanners, bad URLs). Warning: middleware never runs for skipped requests, so Use-based responders on unregistered paths (catch-all 404 pages, static, proxy, healthcheck, rewrite/redirect middleware) and logger/metrics visibility stop working for them. Rate limiters are in the same position: requests to unregistered paths are neither counted nor throttled, and leave no trace in the access log. CORS preflight requests are exempt so cors middleware keeps working. Customize the responses via `ErrorHandler`.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               | `false`                                                                |
...
| <Reference id="strictrouting">StrictRouting</Reference>                               | `bool`                                                          | When enabled, the router treats `/foo` and `/foo/` as different, for parametric routes such as `/users/:id/` as well. Otherwise, the router treats `/foo` and `/foo/` as the same.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | `false`                                                                |
...
| <Reference id="unescapepath">UnescapePath</Reference>                                 | `bool`                                                          | The router always normalizes the request path before matching, as [RFC 3986 Section 6.2.2](https://www.rfc-editor.org/rfc/rfc3986#section-6.2.2) describes: percent-encoded unreserved characters (letters, digits, `-`, `.`, `_`, `~`) are decoded, every other escape keeps its encoding with uppercase hex digits, and `.` and `..` segments are removed, so `/%70rivate` and `/x/../private` both match `/private` while `/a%2Fb` and `//private` stay as sent. When enabled, every escape is decoded, so that routing, `ctx.Path()` and `ctx.Params()` work with the fully decoded path. Only `%XX` escapes are decoded: a `+` stays a `+`, as it is an ordinary path character, and a malformed escape is left as it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `false`                                                                |
...
| <Reference id="certclientfile">CertClientFile</Reference>               | `string`                      | Path of the CA bundle used to verify client certificates, for mTLS on the `CertFile`/`CertKeyFile` path. Ignored when `TLSConfig` is set: that config is served as supplied and this field does not modify it, so a client certificate is required only if its `ClientAuth` is a Require mode, verified against its `ClientCAs`. Fiber logs a warning when both are supplied.             | `""`               |
| <Reference id="certfile">CertFile</Reference>                           | `string`                      | Path of the certificate file. If you want to use TLS, you must enter this field together with `CertKeyFile`; setting only one of them makes `Listen` return `fiber.ErrCertFileAndKeyRequired` instead of silently serving plaintext HTTP.                                                                                    | `""`               |
| <Reference id="certkeyfile">CertKeyFile</Reference>                     | `string`                      | Path of the certificate's private key. If you want to use TLS, you must enter this field together with `CertFile`.                                                                                                                                                                                                          | `""`               |
...
| <Reference id="gracefulcontext">GracefulContext</Reference>             | `context.Context`             | Field to shutdown Fiber by given context gracefully. With `EnablePrefork` it stops only the process it is canceled in; see [Prefork](#prefork).                                                                                                                                                                              | `nil`              |
| <Reference id="ShutdownTimeout">ShutdownTimeout</Reference>             | `time.Duration`               | Specifies the maximum duration to wait for the server to gracefully shutdown. When the timeout is reached, the shutdown stops waiting, without closing the connections that are still active, and the `context.DeadlineExceeded` error is passed to the `OnPostShutdown` callback. A zero value applies the default; set a negative value to disable the timeout and wait indefinitely. | `10 * time.Second` |
...
Each prefork process handles shutdown on its own. A `GracefulContext` canceled in a child drains that child's connections, honoring `ShutdownTimeout`. Canceled in the master, it only runs the master's shutdown hooks: the master never serves requests itself and keeps supervising the children until it exits, and a child whose master is gone exits immediately, without draining. To stop a prefork deployment gracefully, deliver the signal to every process, for example to the whole process group as `systemd` does by default, so that each child's `GracefulContext` observes it. Explicit `Shutdown*` calls behave the same way: they only affect the process they run in.
...
Use `TLSConfig` to configure mutual TLS by setting `ClientAuth` to a Require mode and `ClientCAs` to the roots it verifies against. This replaces `CertClientFile` when you manage TLS configuration directly.
...
ShutdownWithTimeout stops waiting once the timeout expires and returns `context.DeadlineExceeded`; it does not close connections that are still active.

ShutdownWithContext does the same, bounded by the context instead of a timeout. Shutdown hooks will still be executed, even if an error occurs during the shutdown process, as they are deferred to ensure cleanup happens regardless of errors.
...
Call `Shutdown*` from a separate goroutine while `Listen` / `Listener` runs, and keep the process alive until `Shutdown*` returns. Set a non-zero `ReadTimeout`: a connection that has not sent its first request yet, such as a browser preconnect, otherwise holds the shutdown until the timeout, or forever with `Shutdown`. Long-lived streams should end once `c.RequestCtx().Done()` is closed.

`ListenConfig.ShutdownTimeout` is applied only when Fiber runs its **built-in** graceful shutdown path (for example with `GracefulContext`). If you call `app.ShutdownWithTimeout` yourself, that argument is the one that applies. On that built-in path `Listen` / `Listener` returns only once the shutdown has finished, so the caller may exit as soon as it returns; an explicit `Shutdown*` call still needs the process kept alive until it returns. That wait includes the `OnPreShutdown` and `OnPostShutdown` hooks, which `ShutdownTimeout` does not bound. The `OnPreShutdown` and `OnPostShutdown` hooks run exactly once per shutdown, whether it was started by the context or by an explicit `Shutdown*` call, and not at all when `Listen` fails before serving.
