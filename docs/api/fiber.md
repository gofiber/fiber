...
| <Reference id="trustproxy">TrustProxy</Reference> | `bool` | When set to true, fiber will check whether proxy is trusted using TrustProxyConfig.Proxies. <br /><br />By default `c.Protocol()`, `c.IP()`, and `c.Hostname()` read the `Forwarded` header and fall back to the `X-Forwarded-*` headers. If the remote address isn't trusted, these methods use connection information instead. | `false` |
