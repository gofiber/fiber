...
| <Reference id="unescapepath">UnescapePath</Reference>                                 | `bool`                                                          | The router always normalizes the request path before matching, as [RFC 3986 Section 6.2.2](https://www.rfc-editor.org/rfc/rfc3986#section-6.2.2) describes: percent-encoded characters that cannot change the structure of the path are decoded and `.`, `..` and empty segments are removed, so `/%70rivate`, `/x/../private` and `//private` all match `/private`. Reserved characters such as an encoded slash (`%2F`) stay encoded. When enabled, the reserved characters are decoded as well, so that routing, `ctx.Path()` and `ctx.Params()` work with the fully decoded path. Only `%XX` escapes are decoded: a `+` stays a `+`, as it is an ordinary path character, and a malformed escape is left as it is                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   | `false`                                                                |
