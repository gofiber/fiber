...
| <Reference id="preforkrecoverinterval">PreforkRecoverInterval</Reference> | `time.Duration`             | Defines how long the prefork master waits before restarting a crashed child process. Only applies when prefork is enabled.                                                                                                                                                                                                    | `0`                |
| <Reference id="preforkshutdowngraceperiod">PreforkShutdownGracePeriod</Reference> | `time.Duration`       | Defines how long the prefork master waits for child processes to exit after shutdown before killing them. Only applies when prefork is enabled.                                                                                                                                                                               | `5 * time.Second`  |
