...
3. Setting a Cookie: The serialized data is stored in a special cookie (named fiber_flash) that will be sent to the client. The cookie is set `HttpOnly`, and `Secure` whenever the request itself arrived over TLS, so the payload stays out of `document.cookie` and off plaintext connections. It is hex-encoded rather than encrypted or signed, so treat everything in it as visible to the client.
