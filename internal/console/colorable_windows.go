...
// The Windows implementation is derived from github.com/mattn/go-colorable
// which is distributed under the MIT License. The license text is included in
// the repository at internal/console/LICENSE-go-colorable.
...
// newColorable returns a writer that handles escape sequences from the provided file.
func newColorable(file *os.File) io.Writer {
	...
	if IsTerminal(file.Fd()) {
		...
		if r, _, _ := procGetConsoleModeColorable.Call(file.Fd(), uintptr(unsafe.Pointer(&mode))); r != 0 {
			...
			if _, _, err := procSetConsoleMode.Call(file.Fd(), uintptr(mode|cENABLE_VIRTUAL_TERMINAL_PROCESSING)); err == nil {
				return file
			}
		}
...
// ColorableStdout returns a writer which handles escape sequences for stdout.
func ColorableStdout() io.Writer {
	return newColorable(os.Stdout)
}

// newColorableStderr returns a writer which handles escape sequences for stderr.
