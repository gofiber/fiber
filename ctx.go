...
func (c *DefaultCtx) configDependentPaths() {
	...
	// If CaseSensitive is disabled, we lowercase the original path in a single
	// fused copy+lower pass instead of copying and then lowercasing separately.
	if c.app.config.CaseSensitive {
		c.detectionPath = append(c.detectionPath[:0], c.path...)
	} else {
		c.detectionPath = appendLowerASCII(c.detectionPath, c.path)
	}
