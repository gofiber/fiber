...
type (
	...
	Ctx interface {
		...
		Render(name string, bind interface{}, layouts ...string) (err error)
		...
		SendStream(stream io.Reader, size ...int)
...
// Fasthttp return the Fasthttp Request Context
func (ctx *ctx) Fasthttp() *fasthttp.RequestCtx {
	return ctx.fasthttp
}
...
func (ctx *ctx) setFasthttp(fctx *fasthttp.RequestCtx) {
	ctx.fasthttp = fctx
}
