...
func Benchmark_Route_Match(b *testing.B) {
	...
	parsed := parseRoute("/user/keys/:id", defaultRegexHandler)
	route := &Route{
		use:         false,
		root:        false,
		star:        false,
...
func Benchmark_Route_Match_Star(b *testing.B) {
	...
	parsed := parseRoute("/*", defaultRegexHandler)
	route := &Route{
		use:         false,
		root:        false,
		star:        true,
...
func Test_NextCustom_MethodNotAllowed(t *testing.T) {
	...
	useRoute := &Route{use: true, path: "/foo", Path: "/foo", routeParser: parseRoute("/foo", defaultRegexHandler)}
...
func Test_NextCustom_SkipMountAndNoHandlers(t *testing.T) {
	...
	app := newCustomApp()
	m := app.methodInt(MethodGet)
	mountR := &Route{path: "/skip", Path: "/skip", routeParser: parseRoute("/skip", defaultRegexHandler), mount: true}
	empty := &Route{path: "/foo", Path: "/foo", routeParser: parseRoute("/foo", defaultRegexHandler)}
